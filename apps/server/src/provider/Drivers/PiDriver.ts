/**
 * PiDriver — `ProviderDriver` for the Pi runtime (`pi --mode rpc`).
 *
 ...
 * `PiSettings`. pi is an ambient-binary provider with codex-parity
 * package-managed maintenance (`@earendil-works/pi-coding-agent` on npm;
 * `pi update --self` is deliberately not used — one proven code path, no
 * pi.dev dependency — see wayfinder #45).
 *
