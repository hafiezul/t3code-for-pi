...
Issues live in GitHub Issues (`hafiezul/t3code-for-pi`, accessed via the `gh` CLI); external PRs are not a triage surface. See `docs/agents/issue-tracker.md`.
...
Single-context — one `CONTEXT.md` and `docs/adr/` at the repo root (neither exists yet; skills proceed silently until created). See `docs/agents/domain.md`.
